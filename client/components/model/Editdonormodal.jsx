...
import { updateDonor } from "../../store/donorSlice";

const Editdonormodal = ({ onClose, id }) => {
  const dispatch = useDispatch();
  const allData = useSelector((state) => state.donor.donors);

  const oldDonor = allData.filter((donor) => donor._id === id);
  const [form, setForm] = useState(oldDonor[0]);
  ...
  const submitForm = (e) => {
    ...
    delete form.__v;
    dispatch(updateDonor({ payload: form, id: id }));
    onClose();
  };
  return (
